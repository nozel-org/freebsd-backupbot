...
* Optional compression: gzip, bzip2, xz, lz4, zstd, lrzip, lzip, lzop, pigz, pbzip2 and plzip.
* Optional symmetrical encryption of backups.
* Optional file splitting of (large) file backups.
...
Backups can be compressed automatically with `gzip`, `bzip2`, `xz`, `lz4`, `zstd`, `lrzip`, `lzip`, `lzop`, `pigz`, `pbzip2` or `plzip` by setting `BACKUP_FILES_COMPRESSION=''` to:
...
Compression can be disabled by setting `BACKUP_FILES_COMPRESSION=''` to `0`.

Especially databases can often benefit greatly from compression, but in most cases it's also worthwhile to compress files. Option 1 `gzip` is a great starting point because it has a decent balance between speed and compression ratio, while also being part of FreeBSD base. The biggest drawback is `gzip` being single threaded, which is especially apparent on CPUs with a low clock speed. Other compression algorithms might offer better ratios and (much) higher speed, but often require third-party packages to be installed. Option 101 `pigz` for example is a very fast parallel `gzip` implementation, but requires `pigz` to be installed on the server.

For a comparison limited to `gzip`, `bzip2` and `xz` on a relatively slow server cpu, checkout this [Discussion](https://github.com/nozel-org/freebsd-backupbot/discussions/12).
...
#### 3.3.6 Backup file splitting

Sometimes large backups can be unwieldy, for example when you need to move them to another server. Backupbot supports the splitting of backup files by setting the `BACKUP_FILES_SPLIT=''` configuration parameter to a amount of GB as the size of each individual backup part. So if you want to split a large backup in to files of 50 GB each, you set this configuration parameter to `50`. This will create multiple `backup-file.extension.partxx` files that can later be combined again with `cat backup-file.extension.part* > backup-file.extension`.

File splitting can be disabled by setting `BACKUP_FILE_ENCRYPTION='NO'`.
...
| `BACKUP_FILES_COMPRESSION=''` | Set to `1` (gzip), `2` (bzip2) or `3` (xz) to enable compression. Set to `0` to disable compression. See 3.3.4 for more options. |
| `BACKUP_FILES_ENCRYPTION=''` | Set to secret/passphrase to enable encryption. Set to `NO` to disable encryption. |
| `BACKUP_FILES_SPLIT=''` | Set to number in GB to enable file backup splitting. |
...
Backups can be compressed automatically with `gzip`, `bzip2`, `xz`, `lz4`, `zstd`, `lrzip`, `lzip`, `lzop`, `pigz`, `pbzip2` or `plzip` by setting `BACKUP_MYSQL_COMPRESSION=''` to:
...
Compression can be disabled by setting `BACKUP_MYSQL_COMPRESSION=''` to `0`.
...
| `BACKUP_MYSQL_COMPRESSION=''` | Set to `1` (gzip), `2` (bzip2) or `3` (xz) to enable compression. Set to `0` to disable compression. See 3.4.4 for more options. |
